...
pca_model <- prcomp(trait_matrix, center = TRUE, scale. = TRUE)
...
ggplot(cluster_plot, aes(PC1, PC2, color = cluster)) +
...
model_results_clean <- model_results %>%
  ...
  mutate(
    term = case_when(
      ...
      term == "cluster2" ~ "Cluster 2",
      term == "cluster3" ~ "Cluster 3",
...
gtsave(model_results_table, "cluster_model_results.png")
...
gtsave(cluster_summary_final, "cluster_summary_final.png")
...
anova_df <- as.data.frame(anova_results) %>%
  ...
  mutate(
    Model = c("Baseline Model", "Cluster Interaction Model"),
    Parameters = Df,
    AIC = round(AIC, 0),
    BIC = round(BIC, 0),
...
anova_table <- anova_df %>%
  ...
  tab_header(
    title = "Model Comparison: Baseline vs Cluster Interaction Model",
...
coors_effects_plot <- ggplot(coors_effects,
